...
| What it shows | Why |
|---|---|
| The provider and the exact model being asked | `NVIDIA · nvidia/nemotron-3.5-lightning-30b-a3b`, so you know which key is being spent and which model to blame |
...
| `--nvidia-model` | `nvidia/nemotron-3.5-lightning-30b-a3b` | Model for NVIDIA NIM. Models are retired *and un-provisioned* constantly. `https://integrate.api.nvidia.com/v1/models` lists more than your account can call — several listed ids answer `404 Function ...: Not found for account`, and others accept the request and then hang. Pick with `tools/bench_llm.py`, not by reading the list. |
