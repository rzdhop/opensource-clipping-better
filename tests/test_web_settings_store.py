...
def test_default_path_is_not_under_outputs(monkeypatch):
    ...
    # data/, not .local/: it is gitignored, excluded from the image (.dockerignore,
    # so the keys never bake into a layer), and bind-mounted by compose, so a
    # container restart keeps the values. It already holds the API token.
    assert path.endswith("/data/settings.json")
