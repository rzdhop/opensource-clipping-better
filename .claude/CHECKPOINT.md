## CURRENT TASK — AI Story phase 0: the foundation (IN PROGRESS — stage 11 done)
...
- **Current stage / next action:** stage 11 done → **stage 12** (Settings UI: tabs *Providers (LLM/STT)* / *Generation* / *Local hardware* / *Budget* as tab state toggling sections inside the one `<form>` of `Settings.jsx`; Generation tab = five chains read-only with their link rows (keyed / paid / est / adapter), the new keys, per-chain "Test chain" and per-paid-link "Test (est $x)" through `testGenerationChain`, usage today; Local hardware tab = `fetchHardware` profile card, recommendation rows with install hints, ComfyUI/Ollama URL fields + reachability; Budget tab = the stage-6 section; the DEC-057, payload-contract and status/verdict-literal guards stay green; layouts measured at 375/820/1280 on the throwaway).
...
| 11 | settings API (keys, budget fields, test-generation-chain) | **done** (this commit; hash recorded at stage 12) |
