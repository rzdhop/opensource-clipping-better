## CURRENT TASK — AI Story phase 0: the foundation (IN PROGRESS — stage 10 done)
...
- **Current stage / next action:** stage 10 done → **stage 11** (settings API: new secrets `FAL_KEY`, `OPENAI_API_KEY`, `CLOUDFLARE_API_TOKEN`, `CLOUDFLARE_ACCOUNT_ID`, `POLLINATIONS_API_KEY` in `PERSISTED/SECRET_KEYS` + `SettingsRequest/Response` `_set` flags; non-secret `LOCAL_COMFYUI_URL`, `LOCAL_OLLAMA_URL`; `GET /api/settings` reports each chain's links (keyed / paid / allowed / adapter) + usage today + spend today; `POST /api/settings/test-generation-chain {kind, link?}` in `routes/settings.py` sharing `_CHAIN_TEST_LOCK` (rows = LinkProbe fields + est_usd/paid/allowed/artifact, verdict ready | paid_only | blocked | no_adapter, one paid call max and only with `link`); `GET /api/settings/chain-test-file/{name}` through the signed-URL mechanism; compose `extra_hosts` + passthrough; `.env.example` 'Generation providers' block; TestClient tests fail-first).
...
| 10 | hardware profiler + `GET /api/hardware` | **done** (this commit; hash recorded at stage 11) |
