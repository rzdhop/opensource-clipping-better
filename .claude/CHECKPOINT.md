## CURRENT TASK — AI Story phase 0: the foundation (IN PROGRESS — stage 7 done)
...
- **Current stage / next action:** stage 7 done → **stage 8** (TTS + vision adapters: `clipping/providers/tts.py` with `edge/<voice>` through `voiceover._synthesize_async` (no fork; `edge_tts` imported inside the call), `gemini/flash-lite-tts` via REST, `local/piper|kokoro|chatterbox` probed and imported inside functions with an extras group `[local-tts]`; `clipping/providers/vision.py` with `gemini/flash-lite` + `openrouter/<free>` through `registry.build_client` (chat with image parts) and `local/ollama-vision`; `clipping/aistory/templates/voices.json` seed; a test asserts none of the optional packages is in `sys.modules` after import — fail-first).
...
| 7 | image adapters | **done** (this commit; hash recorded at stage 8) |
