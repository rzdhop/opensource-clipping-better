## CURRENT TASK — AI Story phase 0: the foundation (IN PROGRESS — stage 12 done)
...
- **Current stage / next action:** stage 12 done → **stage 13** (deploy + Tier-2: `sudo docker compose rm -sfv backend && docker compose up -d --build backend` only when no job runs — check `GET /api/health` jobs_running first; then the plan §5 script with the human: mode switch + redirects on the phone, paste `FAL_KEY` → badge, Test chain on IMAGE_EDIT_CHAIN with paid off (refusals, no call), allow paid → Test `fal/seedream-4-edit` once (image, ledger, spend, `data/usage.json` byte-identical), TTS_CHAIN Edge sample played, VIDEO_CHAIN no adapter yet, Local hardware = `container_no_gpu`, Clone & Rerun of `b37b36a9b34e` under `/clips/job/…`; the human acks the substitute in chat).
- **Tier-1 after stage 12 (15:30 UTC):** local **1876 passed / 1 skipped**; CI env **1677 passed / 172 skipped**; compileall clean; vite build green. Layouts on the throwaway (worktree at `c5a7995` + the stage-12 files, `DISABLE_AUTH=1`, 127.0.0.1:8010): `/settings` at **375, 820, 1280 px**, every tab, `scrollWidth == clientWidth`; the hardware tab loaded `cpu_only`; a VIDEO chain test rendered five `no adapter yet` rows, a TTS chain test rendered edge/local `unreachable` with their install hints and gemini `no_key`.
...
| 12 | settings UI (four tabs) | **done** (this commit; hash recorded at stage 13) |
