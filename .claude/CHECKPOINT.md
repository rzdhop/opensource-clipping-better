## CURRENT TASK — AI Story phase 0: the foundation (IN PROGRESS — stage 6 done)
...
- **Current stage / next action:** stage 6 done → **stage 7** (image adapters in `clipping/providers/images.py`: cloudflare/flux-1-schnell, pollinations/*, gemini/nano-banana-2[-lite] via REST with reference images, fal/flux-schnell, fal/seedream-4-edit, fal/flux-kontext-pro via the fal queue API, openai/gpt-image-2-low via the installed SDK; each `estimate` → `budget.check` before a paid call, `limits.acquire` for free calls, exceptions with `.status_code`, `probe()`, injectable `transport`; fake-transport tests fail-first incl. a 404 model-gone swap and "allow_paid off → transport never called").
...
| 6 | budget + cost ledger (caps 1.00 / 3.00 / 10.00) | **done** (this commit; hash recorded at stage 7) |
