## CURRENT TASK — AI Story phase 0: the foundation (IN PROGRESS — stage 13: live fixes committed, Tier-2 steps 2–9 wait for the human)
...
- **Current stage / next action:** stage 13 (deploy + Tier-2). The container was rebuilt from `533647c` at 14:45 UTC and restarted at 18:33 UTC to load the stage-13 fixes (the backend runs the bind-mounted Python; `voiceover` is cached in the process, so a code fix needs `sudo -n docker compose restart backend` when `GET /api/health` says 0 jobs). **Next: the human runs plan §5 steps 2, 4, 5 (phone playback), 8, 9** (they need the fal.ai key and the phone): paste `FAL_KEY` in Settings → Generation (badge, survives reload); with `allow_paid` off, Test chain on IMAGE_EDIT_CHAIN → refusals with the numbers and no call; turn `allow_paid` on (Budget tab → profile `one_dollar`) and press **Test (est $0.030)** on `fal/seedream-4-edit` ONCE → one 9:16 image in the row, one entry in `data/chain_test_ledger.json`, $0.03 in `data/spend.json`, `data/usage.json` byte-identical before/after (copy it aside first, then `cmp`); play the Edge sample on the phone; Clone & Rerun of `b37b36a9b34e` completes under `/clips/job/<new id>`; the phone confirms 375 px. **The human must acknowledge the Tier-2 substitute in chat** (no E2E suite exists). Then stage 14 (docs + decisions, one commit), then ask before pushing (github_osc_better key with `-F /dev/null`, see memory), then phase 1.
- **Tier-1 after stage 13 fixes (18:30 UTC):** local **1879 passed / 1 skipped**; CI env **1679 passed / 173 skipped**; compileall clean; vite build green.
- **Stage 13 evidence (live, deployed container):** served title "rzdhop AI"; `/job/b37b36a9b34e` redirects to `/clips/job/…` with 7 playing clips; `/api/hardware` in the container = `container_no_gpu` with the host.docker.internal hint (errors recorded, not hidden: "pynvml: NVML Shared Library Not Found"); IMAGE_EDIT test with paid off = verdict blocked, gemini rows "refused: est $0.034/$0.067 … allow_paid is off (today $0.00 of $3.00)", fal rows no_key, **no network call**; VIDEO = no_adapter ×5; IMAGE test: `pollinations/flux` answered in 3.4 s (sample under `outputs/_chain_test/`); TTS test: edge 1.6 s, `gemini/flash-lite-tts` 2.1 s (WAV) — model id `gemini-3.8-flash-lite-tts` confirmed live; VISION test: `gemini/flash-lite` described the reference image, `openrouter/qwen/qwen3.8-27b:free` answered 429 "temporarily rate-limited upstream" (A-032 evidence), `local/ollama-vision` unreachable; `data/usage.json` counted edge 1, gemini 2, openrouter 1, pollinations 1; no spend, no ledger. **After the stage-13 fixes (18:34 UTC):** TTS test verdict ready in 4.4 s, the Edge timing file = source `tts_word_timestamps`, **10 words, 2.9 s** (was 0 words / 0.0 s); `GET /api/settings` vision `gemini/flash` = blocked "refused: est $0.000 … allow_paid is off" (was "allowed").
- **Stage 13 fixes (found by the live run):** `voiceover._word_boundary_kwargs()` — edge-tts 7.2.8 defaults `Communicate` to `SentenceBoundary`, so the shared core saw no WordBoundary chunk and **every voice-over lost its word timings, including the Clips voice-over feature**; the core now passes `boundary="WordBoundary"` when the argument exists (older edge-tts: no argument, unchanged). `tts.audio_duration()` (ffprobe) + `probe_duration=`: with no cues the timing file carries the measured length and a ⚠️ line, never 0.0. `routes/settings._link_summary` uses the adapter's own estimate (`_summary_request`) and never reports a paid link allowed while `allow_paid` is off (gemini/flash's token estimate rounded to $0.000 and the row said allowed).
- **Tier-1 after stage 12 (~14:40 UTC; an earlier note said 15:30 UTC, which was wrong — the commit is 14:44 UTC):** local **1876 passed / 1 skipped**; CI env **1677 passed / 172 skipped**; compileall clean; vite build green. Layouts on the throwaway (worktree at `c5a7995` + the stage-12 files, `DISABLE_AUTH=1`, 127.0.0.1:8010): `/settings` at **375, 820, 1280 px**, every tab, `scrollWidth == clientWidth`; the hardware tab loaded `cpu_only`; a VIDEO chain test rendered five `no adapter yet` rows, a TTS chain test rendered edge/local `unreachable` with their install hints and gemini `no_key`.
- **Stage 12 contract:** `Settings.jsx` tabs `SETTINGS_TABS` (providers / generation / hardware / budget, last one in `localStorage rzc_settings_tab`) as tab state toggling `settings-grid` blocks inside the one `<form>`; Generation tab = the five key inputs + `ChainLinksPanel` (one card per chain: env, source, chain string, link rows with `linkGlyph` + `.link-chip` facts, per-chain "Test chain", per allowed paid link "Test (est $x)" through `testGenerationChain({ kind, link })`, `GenerationChainResult` rows with `GEN_STATUS_GLYPH` / `GEN_VERDICT_STYLE`, `<img>`/`<audio>` samples) + "Free allowance today"; Local hardware tab = `HardwarePanel` (`fetchHardware`, profile card, probe errors, recommendations with install hints and workflow chips, "Probe again") + the `LOCAL_COMFYUI_URL` / `LOCAL_OLLAMA_URL` inputs + System Info; Budget tab = the stage-6 section. CSS: `.settings-tabs` / `.settings-tab.active`, `.link-chip` (NOT `.chip`, which is the job-id chip), and the grid fix `minmax(min(320px, 100%), 1fr)` / `minmax(0, 1fr)` on phones + `.settings-section { min-width: 0; overflow-wrap: anywhere }` so no unbreakable token widens the page again. Guards: `tests/test_settings_tabs.py` (tabs, remembered tab, every generation status/verdict drawn, chain + per-link test, hardware fed by the API + URL fields, sections in their tabs).
- **Tier-1 after stage 11 (14:52 UTC):** local **1871 passed / 1 skipped**; CI env **1672 passed / 172 skipped**; compileall clean; vite build green.
- **Stage 11 contract:** `settings_store` persists `FAL_KEY`, `OPENAI_API_KEY`, `CLOUDFLARE_API_TOKEN`, `CLOUDFLARE_ACCOUNT_ID`, `POLLINATIONS_API_KEY` (secrets) and `LOCAL_COMFYUI_URL`, `LOCAL_OLLAMA_URL`; `SettingsRequest` gains the seven fields, `SettingsResponse` the five `_set` flags, the effective local URLs, `generation_chains` ({kind: {env, source, chain, links: [{label, provider, model, paid, keyed, missing_keys, adapter, allowed, est_usd, reason, env_keys, signup_url}], error}}) and `usage_today`; `POST /api/settings/test-generation-chain {kind, link?, chain?}` → `GenerationChainTestResponse {kind, chain, verdict ready|paid_only|blocked|no_adapter, results: [GenerationLinkResult status ok|failed|no_key|no_adapter|unreachable|refused|skipped, paid, est_usd, allowed, reason, note, latency_seconds, artifact_url, artifact_kind, …], elapsed_seconds, message, tested_link}` under `_CHAIN_TEST_LOCK` + `_CHAIN_TEST_CEILING_SECONDS`; free/local links run one by one through `run_generation_chain` with the limiter; a paid link is reported unless named in `link`, then called once after `budget.check`, booked in `CHAIN_TEST_LEDGER` (`data/chain_test_ledger.json`, step `chain_test`, the provider's API model id) and `budget.record`; samples written under `outputs/_chain_test/` (reserved id, never a job) and served by the existing signed outputs route (`auth.media_url("_chain_test", name)`); a bundled reference PNG is generated with the stdlib for edit/vision tests; `_TRANSPORT` is the injectable transport; `clipping/providers/adapters.load_all()` registers every phase-0 adapter; compose passes the 17 generation/budget variables and adds `extra_hosts: host.docker.internal:host-gateway`; `.env.example` documents them; `Settings.jsx` carries the five key inputs with badges (DEC-057); `api.js` has `testGenerationChain` and `fetchHardware`. DEVIATION from the plan text: no separate `chain-test-file` route — the existing signed outputs route serves the samples under the reserved `_chain_test` id (smaller auth surface).
- **Tier-1 after stage 10 (14:22 UTC):** local **1855 passed / 1 skipped**; CI env **1667 passed / 161 skipped**; compileall clean. Live probe on this host: `cpu_only`, 23.4 GB RAM, 80 GB free, ComfyUI unreachable, **Ollama 0.34.0 running on 127.0.0.1:11434 with no models pulled** (a real daemon on the VPS).
- **Stage 10 contract:** `clipping/aistory/hardware.py`: `PROFILES`, `HardwareProfile` dataclass (gpu_name, vram_gb, vram_free_gb, ram_gb, disk_free_gb, backend, in_container, comfyui{reachable, note, version, url, devices}, ollama{reachable, note, url, models}, profile, recommendations, sources, errors, probed_at), parsers `parse_nvidia_smi` / `parse_system_profiler` / `parse_rocm_smi` / `parse_wmic` / `parse_meminfo` / `parse_comfy_system_stats` (fixtures under `tests/fixtures/hardware/`), `classify(vram_gb, backend, in_container)` (mps → apple_mps; no GPU → cpu_only / container_no_gpu; <8 low, <16 mid, <24 high, ≥24 pro), `probe(run=, read_file=, platform=, importer=, container=, disk_free_gb=, comfy=, ollama=, sysctl_memsize=, env=)` in spec order with ComfyUI `/system_stats` authoritative, `recommendations_for(profile)` (rows task/model/install_hint/workflow; container rows carry the host.docker.internal hint), `to_dict`; `web/api/routes/hardware.py` `GET /api/hardware?refresh=` token-gated, cached 60 s, probes in a thread with the saved Settings env; `app.py` includes the router.
- **Tier-1 after stage 9 (14:07 UTC):** local **1823 passed / 1 skipped**; CI env **1636 passed / 160 skipped**; compileall clean.
- **Stage 9 contract:** `local_comfyui.ComfyUIClient(base_url, transport=, client_id=)` with `system_stats()`, `object_info()` (cached), `reachable()`, `queue_prompt(graph)` (node_errors → `ComfyUIError`), `upload_image(path, subfolder=)` → LoadImage name, `history(id)`, `view(file)`, `wait(id, use_ws=, ws_factory=, budget=)` (websocket progress then `/history` polling, ↩ line on fallback), `run(graph)`; `ws_frames(base_url, client_id)` stdlib RFC 6455 reader (text frames as JSON events, binary previews skipped, ping answered, close ends); templates `TEMPLATES` = t2i_flux2_klein (low), edit_flux2_klein_multiref (mid, 4 ref slots, ReferenceLatent chain), edit_qwen_image (high, GGUF, 1 slot) under `clipping/aistory/templates/workflows/` (`comfy_workflow_v1`: placeholders, `requires` [{node, field, file, dir}], `ref_slots`/`ref_nodes`, `output_node`, `graph`); `load_template`, `render_template(template, values)` (typed placeholders, unused reference slots repeat the last reference, too many → error), `validate_template(template, object_info)` (missing custom node / model file named); adapter `COMFYUI` registered for image + image_edit under `local` (`extra["template"]` picks one; validation before `/prompt`). Runner protocol widened: `probe(link, *, credentials, **kw)` / `generate(..., **kw)` receive `env` for local links and `transport` when injected. `local_ollama.OllamaClient.chat(format=)` for JSON schemas.
- **Tier-1 after stage 8 (13:53 UTC):** local **1802 passed / 1 skipped**; CI env **1615 passed / 160 skipped**; compileall clean.
- **Stage 8 contract:** `tts.EDGE` (`edge/<voice>`: `voiceover._synthesize_async` through `asyncio.run`, `synthesize=` injectable; mp3 + `.srt` + `line_timing_v1` json with word timestamps, source `tts_word_timestamps`), `tts.GEMINI_TTS` (`gemini/flash-lite-tts` → `gemini-3.8-flash-lite-tts`, `responseModalities: [AUDIO]`, `prebuiltVoiceConfig.voiceName` default `Kore`, PCM → WAV, source `audio_duration_only`), `tts.LOCAL_TTS` (`local/piper|kokoro|chatterbox`: `_installed()` probe, engines imported inside `_synthesize_*`, hint `pip install 'rzdhop-ai[local-tts]'`; unknown engine → 404), `tts.load_voices()` / `voices_for(provider, lang)` over `clipping/aistory/templates/voices.json` (`voices_v1`: edge fr/en, gemini prebuilt, piper, kokoro, chatterbox); `vision.GEMINI_VISION` / `OPENROUTER_VISION` (`llm.build_client` compat chat with `image_url` data URLs; `GEMINI_VISION_MODELS` flash-lite → `registry.GEMINI_DEFAULT_MODEL`, flash → `gemini-3.5-flash`; text in `meta["text"]`, `paths=()`; paid estimate = prompt tokens + 258 per image), `vision.OLLAMA_VISION` (`local_ollama.OllamaClient(base_url).tags()/chat(model, messages, images=)`, default model `gemma3:4b`, probe reports unreachable / not pulled); `pyproject.toml` `[local-tts]` = piper-tts>=1.8.0,<2, kokoro>=0.9.4,<1, chatterbox-tts>=0.1.7,<1 (PyPI 2026-09-25).
- **Tier-1 after stage 7 (13:40 UTC):** local **1782 passed / 1 skipped**; CI env **1595 passed / 160 skipped**; compileall clean.
- **Stage 7 contract:** `transport.Response(status, headers, body)`, `urllib_transport`, `request_json` / `request_bytes` (4xx/5xx → `HttpStatusError(status_code, url, detail)`), `APIConnectionError` / `APITimeoutError` (named so `errors.classify` retries them), `read_b64`, `data_url`, `write_output(out_dir, name, data, ext)` (atomic). `images.py`: `CLOUDFLARE_MODELS`, `GEMINI_MODELS`, `FAL_APPS` (image + video ids), `OPENAI_MODELS`; adapters `CLOUDFLARE`, `POLLINATIONS`, `GEMINI` (REST `generateContent`, `responseModalities: [IMAGE]`, `imageConfig.aspectRatio`, `inline_data` references), `FAL` (queue API submit → status polls every 2 s within a 300 s budget → response → media fetch; inputs per app), `OPENAI` (SDK inside the call, `max_retries=0`, `images.generate` / `images.edit`); registered for image (cloudflare, pollinations, gemini, fal, openai) and image_edit (gemini, fal, openai); `GenRequest.out_dir` required; `extra["name"]` names the file; an unknown model raises a 404 `HttpStatusError` so the runner swaps or moves on. Gating stays in the runner (DEC-097): adapters never check the budget themselves.
- **Tier-1 after stage 6 (13:32 UTC):** local **1760 passed / 1 skipped**; CI env **1573 passed / 160 skipped**; compileall clean; vite build green (budget section in the bundle).
- **Stage 6 contract:** `budget.ALLOW_PAID` (False), `PER_EPISODE_CAP_USD` 1.00, `DAILY_CAP_USD` 3.00, `PER_STORY_CAP_USD` 10.00, `BUDGET_PROFILE` "" → `resolve_profile(setting, allow_paid)` (free / one_dollar / explicit), `Budget` namedtuple, `budget_from_env(env)` (garbage amount raises naming the variable), `load_profiles()` / `profile_settings(name)` from `clipping/aistory/templates/budget_profiles.json` (spec §8.5.1 verbatim, validated), `check(est, link, *, budget, day_spent, ep_spent, story_spent)` raising `BudgetRefused` with the numbers (free est never refused; order: switch → episode → day → story), `DailySpend` (`data/spend.json` or `SPEND_PATH`, `spend_v1`, atomic), `record(est)`, `day_spent()`; `clipping/aistory/ledger.CostLedger(path)` (`cost_ledger_v1`, `append(step, provider, model, unit, qty, est_usd, paid, ep)`, `entries()`, `totals(ep)`, `episode_view(ep, out_path)`). Five places: `config.ALLOW_PAID/PER_EPISODE_CAP_USD/DAILY_CAP_USD/PER_STORY_CAP_USD/BUDGET_PROFILE` + CLI group "Budget (AI Story)" (`--allow-paid`, `--per-episode-cap-usd`, `--daily-cap-usd`, `--per-story-cap-usd`, `--budget-profile`) + `Config` kwargs; `SettingsRequest` (Optional) / `SettingsResponse` (`allow_paid`, caps, `budget_profile`, `effective_budget_profile`, `spend_today_usd`); `config_adapter` (`env_flag` / new `env_float`); `routes/settings.py` (`_budget_fields`, PUT validates and stores, switch clears like ALLOW_SLOW_CHAIN); `Settings.jsx` "💰 Budget" section (toggle, three amounts, profile select, spent today); `PERSISTED_KEYS` + `.env.example` block.
- **Tier-1 after stage 5 (13:26 UTC):** local **1737 passed / 1 skipped**; CI env **1550 passed / 160 skipped**; compileall clean.
- **Stage 5 contract:** `pricing.PRICES_AS_OF`, `PRICES` (link → `Price(unit, usd, note, per_megapixel)`), `price_for(link)`, `estimate(link, qty, *, width, height) -> Estimate(link, unit, qty, price_usd, est_usd, paid)`, `PriceUnknown`, `price_table()`; `limits.Limit(rpm, rpd)`, `published_limits()`, `limits_from_env(env)` (`LIMIT_<PROVIDER>_RPM/RPD`), `DailyUsage(path, time_fn)` (`usage_v1`, atomic write, UTC-day reset, `try_increment`), `default_usage_path()` (`USAGE_PATH` or `data/usage.json`), `acquire(provider, *, usage, limits, time_fn, sleep_fn) -> None | reason` (RPD first, then RPM through `pacing.Limiter` with per-call clock indirection), `budget_left_today()`, `reset()`.
- **Tier-1 after stage 4 (13:13 UTC):** local **1709 passed / 1 skipped**; CI env **1522 passed / 160 skipped**; compileall clean; `tests/test_preflight.py` unedited (RC-C9).
- **Stage 4 contract (what later stages build on):** `generation.KINDS` = image | image_edit | video | tts | vision; `ENV_NAMES`, `DEFAULT_CHAINS` (spec §8.1, `*` tolerated and stripped), `GEN_PROVIDERS` (cloudflare, pollinations, gemini, fal, openai, openrouter, edge, local + extension points gcloud, elevenlabs; `env_keys` all required, `optional_keys`, `free_tier`, rpm/rpd), `KIND_PROVIDERS`, `PAID_LINKS` + `is_paid(link)` (per link), `FALLBACK_LINKS` (like-for-like swaps only), `parse_generation_chain(kind, spec)`, `chain_from_env(kind, env)`, `local_url(service, env, container=)` (Docker-aware), `GenRequest`/`GenResult` dataclasses, `register_adapter(kind, provider, adapter)` where an adapter has `estimate(link, request)`, `probe(link, *, credentials)`, `generate(link, request, *, credentials, on_log, transport=None)`; `run_generation_chain(kind, chain, request, *, env, allow_paid, route, on_log, budget_check(est, link), limiter.acquire(provider) -> reason|None, adapters, transport, sleep_fn, time_fn, cancel) -> (GenResult, link)` raising `NoRunnableLink(ProviderError)` with `.failures`. Gates in order: route → adapter present → keys → local probe → paid/allow_paid/budget (💸) or limiter (⏳) → attempt (🔁, one retry on RETRY/RATE_LIMITED, ✖ on fatal, ↪ swap on model-unavailable, ✅ on success). `registry.parse_spec/parse_chain` accept `providers=` (default: the LLM table, behaviour unchanged).
