## CURRENT TASK — AI Story phase 0: the foundation (IN PROGRESS — stage 4 done)
...
- **Current stage / next action:** stage 4 done → **stage 5** (pricing + free-tier limits: `clipping/providers/pricing.py` with `PRICES_AS_OF = "2026-09-25"`, `PRICES` per link with unit image|second|char|token, `estimate(link, qty)`; `clipping/providers/limits.py` with `LIMITS` per provider, RPD counters in `data/usage.json` keyed by UTC day checked first, RPM through `pacing.limiter_for`, `acquire(provider)`, `budget_left_today()`; tests fail-first: every paid default link has a price, free links estimate $0, counters with an injected clock and tmp path, a paid link never touches the counters, RPD exhaustion refuses without sleeping).
...
| 4 | generation chain core (`providers/generation.py`) | **done** (this commit; hash recorded at stage 5) |
