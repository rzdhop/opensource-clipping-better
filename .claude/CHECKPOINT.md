## CURRENT TASK — AI Story phase 0: the foundation (IN PROGRESS — stage 5 done)
...
- **Current stage / next action:** stage 5 done → **stage 6** (budget + cost ledger: `clipping/providers/budget.py` with `ALLOW_PAID` off, caps 1.00 / 3.00 / 10.00, `BUDGET_PROFILE` resolved free → one_dollar when paid is on, `clipping/aistory/templates/budget_profiles.json` = spec §8.5.1, `check(est, link, *, ep_spent, story_spent)` raising `BudgetRefused` with the numbers, `record(est)` → `data/spend.json`; `clipping/aistory/ledger.py` `CostLedger`; the five places on the ALLOW_SLOW_CHAIN template (config constant + argparse, `SettingsRequest/Response`, `config_adapter`, `routes/settings.py`, `Settings.jsx`) + `PERSISTED_KEYS` + `.env.example`; five-place agreement test fail-first).
...
| 5 | pricing + free-tier limits | **done** (this commit; hash recorded at stage 6) |
