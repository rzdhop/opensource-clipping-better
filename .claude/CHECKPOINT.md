## CURRENT TASK — AI Story phase 0: the foundation (IN PROGRESS — stage 9 done)
...
- **Current stage / next action:** stage 9 done → **stage 10** (hardware profiler `clipping/aistory/hardware.py`: probes in spec order (pynvml → nvidia-smi → torch.cuda → system_profiler + MPS → rocm-smi → wmic → RAM → disk → ComfyUI `/system_stats` → Ollama `/api/tags`), Docker detection, fixture-tested parsers under `tests/fixtures/hardware/`, `HardwareProfile` → profile `cpu_only|low|mid|high|pro|apple_mps|container_no_gpu` with the §8.2 thresholds, recommendations with install hints and the matching workflow templates; `web/api/routes/hardware.py` `GET /api/hardware` token-gated, cached 60 s; the AST auth guard stays green; live call here → `cpu_only`).
...
| 9 | local ComfyUI / Ollama clients + 3 workflow templates | **done** (this commit; hash recorded at stage 10) |
