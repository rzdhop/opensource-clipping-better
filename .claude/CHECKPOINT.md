## CURRENT TASK — AI Story phase 0: the foundation (IN PROGRESS — stage 8 done)
...
- **Current stage / next action:** stage 8 done → **stage 9** (local clients: `clipping/providers/local_comfyui.py` — `POST /prompt`, `/ws?clientId=` progress through a stdlib RFC 6455 text-frame reader with `/history/{id}` polling as fallback, `/view`, `/system_stats`, `/object_info` validation of every `class_type` a template uses, `/upload/image` multipart for references; the §8.7 placeholder template engine; three templates `t2i_flux2_klein.json`, `edit_flux2_klein_multiref.json`, `edit_qwen_image.json` with `requires` + `min_profile`; `local_ollama.py` gains `format=<schema>` for the LLM link; tests with fake transports and an in-process socket server for the `/ws` reader, fail-first).
...
| 8 | TTS + vision adapters | **done** (this commit; hash recorded at stage 9) |
