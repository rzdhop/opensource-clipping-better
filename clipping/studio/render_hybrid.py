...
# At module level, never inside the frame loop: its renderer cache has to live
# as long as the module does (tests/test_watermark_cache.py).
...
def buat_video_hybrid(
    ...
    # STEP_DETEKSI     = max(0.5, (end_clip - start_clip) / 60.0)   # [OLD] AI checks the face every max 0.5s, or across the (end_clip - start_clip) duration in seconds per minute

    DEADZONE_RATIO   = cfg.track_deadzone if cfg.track_deadzone is not None else 0.15  # 15% of the center area is the safe zone (camera doesn't follow movement)
    # DEADZONE_RATIO   = 0.25  # 25% of the center area is the safe zone (camera doesn't follow movement)
    # DEADZONE_RATIO   = 0.20  # [OLD] 20% of the center area is the safe zone (camera doesn't follow movement)
    ...
    SNAP_THRESHOLD   = cfg.track_snap if cfg.track_snap is not None else 0.25  # If the face jumps > 25% of screen width, assume a person change (hard cut)
    # SNAP_THRESHOLD   = 0.30  # [NEW; NOT USED] If the face jumps > 30% of screen width, assume a person change (hard cut)
    # =======================================================
